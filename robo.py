...
# --- CONFIG SUPABASE ---
# NUNCA insira senhas no código em texto puro.
# Configure a variável DATABASE_URL no seu ambiente de execução (ex: GitHub Actions, Server ou Vercel)
...
def formatar_data_para_sql(data_str):
    ...
    # Extrai o primeiro padrão de data DD/MM/YYYY encontrado na string
    match = re.search(r'(\d{2}/\d{2}/\d{4})', data_str.strip())
    if match:
        try:
...
def salvar_no_banco(concursos):
    ...
    try:
        ...
        for c in concursos:
            ...
            s_min = float(c.get('salario_min')) if c.get('salario_min') else 0.0
            s_max = float(c.get('salario_max')) if c.get('salario_max') else 0.0
            v_insc = float(c.get('valor_inscricao')) if c.get('valor_inscricao') else 0.0

            vals = (
                c['orgao'][:255], 
                c['cidade'][:100], 
                c['status'][:50], 
                c.get('cargos', 'Consultar Edital'),
                s_min, 
                s_max,
                c.get('escolaridade', 'Ver Edital')[:100], 
                v_insc,
                ...
                c['link_oficial'], 
                c.get('link_inscricao', c['link_oficial']), 
                hash_id
            )
        ...
        conn.commit()
        print(f"✅ Sucesso: {novos} concursos salvos/atualizados no Supabase.")
    ...
    try:
        ...
        for box in soup.select('.curso-box'):
            ...
            if any(x in nome.upper() for x in ["MA", "MARANHÃO"]):
                concursos.append({
                    "orgao": normalizar_nome(nome.upper()), 
                    "cidade": "Maranhão", 
                    "status": "Aberto",
        ...
        print(f"⚠️ Falha no scraper do Instituto Legatus: {err}")
    return concursos
...
if __name__ == "__main__":
    ...
    if lista_final:
        ...
        print("📭 Nenhum concurso novo encontrado hoje.")